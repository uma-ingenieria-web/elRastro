...
const productURL = `${
    process.env.NEXT_PUBLIC_BACKEND_CLIENT_PRODUCT_SERVICE ?? "http://localhost:8002"
}/api/v1/products`;
...
function Product({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            ...
            setMap(
                <StaticMap
                    position={[productFetched.owner.location.lat, productFetched.owner.location.lon]}
                ...
                <div className="flex flex-col flex-1">
                    ...
                    <div className="border-t border-b border-gray-300 py-2 mb-4 flex justify-between flex-col">
                        <div className="flex justify-between w-full">
                            <div className="flex flex-col">
                                ...
                                {co2Rate !== null && session && (
                                    <span className="text-gray-600 font-semibold mt-2">CO2 Rate:</span>
                                )}
                            </div>
                            <div className="flex flex-col items-end">
                                ...
                                {co2Rate !== null && session && (
                                    <>
                                        {co2Rate < 0.0001 && session?.user?.id !== product?.owner._id && (
                                        ...
                                        {co2Rate > 0 && session?.user?.id !== product?.owner._id && (
                                            <div className="flex justify-between w-full mt-2">
                                                <span className="font-semibold text-sm">{co2Rate.toPrecision(4)}€</span>
                                            </div>
                                        )}
                                    </>
                                )}
                        ...
                        {!closed ? (
                            <div className="mt-4 flex justify-center">
                                ...
                                <input
                                    ...
                                    className={`border border-gray-300 rounded-md w-32 px-2 py-1 ml-2 ${
                                        userId && !closed ? "cursor-pointer" : "cursor-not-allowed bg-gray-300"
                                    }`}
                            ...
                            <div className="mt-4 flex justify-center font-bold mb-4">You already paid for this product</div>
                        ) : (
                            <Link href={`/checkout/${product?._id}`}>
                                <button className={`text-white px-4 py-1 rounded-md ml-2 cursor-pointer bg-green-500`}>
                                    Go to Checkout
                                </button>
                            </Link>
                        )}
                        <div className="mb-4">
                            {userId &&
                                closed && userId !== product?.owner._id && userId === product?.buyer._id &&
                                ((rating != 0 && (
                                    <>
